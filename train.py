...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from,
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        image, viewspace_point_tensor, visibility_filter, radii, object_feature = render_pkg["render"], render_pkg["viewspace_points"], \
        render_pkg["visibility_filter"], render_pkg["radii"], render_pkg["render_object_feature"]
        ...
        logits = classifier(object_feature)
        ...
        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim_value) + loss_obj
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                cur_size = torch.cuda.memory_allocated() / (10 ** 6)  # gaussians.size()/8/(10**6)
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, loss_obj, l1_loss, iter_start.elapsed_time(iter_end),
...
def prepare_output_and_logger(args, wandb_name):
    if not args.model_path:
        if os.getenv('OAR_JOB_ID'):
            unique_str = os.getenv('OAR_JOB_ID')
        else:
            unique_str = str(uuid.uuid4())
        args.model_path = os.path.join("/vol/teaching/DigitalHuman/Raushan/output/", unique_str[0:10])
...
def training_report(tb_writer, iteration, Ll1, loss, loss_obj, l1_loss, elapsed, testing_iterations, scene: Scene, renderFunc,
                    renderArgs, use_wandb):
    if tb_writer:
    ...
    if use_wandb:
        wandb.log({"train_loss_patches/l1_loss": Ll1.item(),
                   # "train_loss_patches/l1_loss_feature": Ll1_feature.item(), #New
                   "train_loss_patches/l1_loss_object": loss_obj.item(), #New
    ...
    for config in validation_configs:
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            for idx, viewpoint in enumerate(config['cameras']):
                ...
                save_image(osp.join(cur_vis_path, "render_{:04d}.png".format(idx)),
...
if __name__ == "__main__":
    ...
    tb_writer = prepare_output_and_logger(args, wandb_name)
    if wandb_enabled:
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations,
             args.checkpoint_iterations, args.start_checkpoint, args.debug_from, wandb_enabled)
