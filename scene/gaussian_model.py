...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int):
        ...
        self._object_feature = torch.empty(0)
        self.num_object_feature = 16
    ...
    def restore(self, model_args, training_args):
        (self.active_sh_degree,
         ...
         opt_dict,
         self.spatial_lr_scale,
         self._object_feature) = model_args
    ...
    def get_object_feature(self):
        return self._object_feature

    def rewrite_object_feature(self, x):
        self._object_feature = x
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        features[:, 3:, 1:] = 0.0
        self._object_feature = torch.zeros(fused_point_cloud.shape[0], self.num_object_feature, 1).float().cuda()
        ...
        self._object_feature = nn.Parameter(self._object_feature.transpose(1, 2).contiguous().requires_grad_(True))
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._object_feature], 'lr': training_args.object_feature_lr, "name": "object_feature"}
        ]
    ...
    def inpaint_setup(self, training_args, mask3d):
        ...
        def initialize_new_features(features, num_new_points, mask_xyz_values, distance_threshold=0.25,
                                    max_distance_threshold=1, k=5):
            ...
            return new_features['xyz'], new_features['features_dc'], new_features['scaling'], new_features[
                'features_rest'], new_features['opacity'], new_features['rotation']
        ...
        self._features_dc = nn.Parameter(
            torch.cat([set_requires_grad(features_dc_sub, False), set_requires_grad(new_features_dc, True)]))
        self._features_rest = nn.Parameter(
            torch.cat([set_requires_grad(features_rest_sub, False), set_requires_grad(new_features_rest, True)]))
        self._opacity = nn.Parameter(
            torch.cat([set_requires_grad(opacity_sub, False), set_requires_grad(new_opacity, True)]))
        self._scaling = nn.Parameter(
            torch.cat([set_requires_grad(scaling_sub, False), set_requires_grad(new_scaling, True)]))
        self._rotation = nn.Parameter(
            torch.cat([set_requires_grad(rotation_sub, False), set_requires_grad(new_rotation, True)]))
    ...
    def construct_list_of_attributes(self):
        ...
        # Add object features
        for i in range(self._object_feature.shape[1] * self._object_feature.shape[2]):
    ...
    def save_ply(self, path):
        ...
        object_feature = self._object_feature.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
    ...
    def load_ply(self, path):
        ...
        count = sum(1 for name in plydata.elements[0].data.dtype.names if name.startswith("obj_feat_"))
        object_feature = np.stack([np.asarray(plydata.elements[0][f"obj_feat_{i}"]) for i in range(count)], axis=1)
        object_feature = np.expand_dims(object_feature, axis=-1)
        ...
        self._object_feature = nn.Parameter(torch.tensor(object_feature, dtype=torch.float, device="cuda").transpose(1,2).contiguous().requires_grad_(True))
    ...
    def prune_points(self, mask):
        ...
        self._object_feature = optimizable_tensors["object_feature"]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling,
                              new_rotation, new_object_feature):
        d = {"xyz": new_xyz,
             ...
             "object_feature": new_object_feature}
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2, limit_num=-1):
        ...
        new_object_feature = self._object_feature[selected_pts_mask].repeat(N, 1, 1)

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_object_feature)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, limit_num):
        ...
        new_object_feature = self._object_feature[selected_pts_mask]
